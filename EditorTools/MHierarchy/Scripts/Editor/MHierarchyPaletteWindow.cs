...
using static MHierarchy.MHierarchyPalette;
...
{
    public class MHierarchyPaletteWindow : EditorWindow
    {
        void OnGUI()
        {
            ...
            void colors()
            {
                ...
                void color(int i)
                {
                    ...
                    void color()
                    {
                        ...
                        var brightness = i <= MHierarchyPalette.greyColorsCount ? 1.02f : 1.35f;
                        var outlineColor = i <= MHierarchyPalette.greyColorsCount ? Greyscale(.0f, .4f) : Greyscale(.15f, .2f);
        ...
        bool usingDataSO => !gameObjects.Select(r => r.scene).All(r => MHierarchy.dataComponents_byScene.GetValueOrDefault(r) != null);
        IEnumerable<MHierarchyDataComponent> usedDataComponents => MHierarchy.dataComponents_byScene.Where(kvp => kvp.Value && gameObjects.Select(r => r.scene).Contains(kvp.Key)).Select(kvp => kvp.Value);
        ...
        public void Init(List<GameObject> gameObjects)
        {
            void createData()
            {
                if (MHierarchy.data) return;

                MHierarchy.data = ScriptableObject.CreateInstance<MHierarchyData>();

                AssetDatabase.CreateAsset(MHierarchy.data, GetScriptPath("MHierarchy").GetParentPath().CombinePath("mHierarchy Data.asset"));

            }
            void createPalette()
            {
                if (MHierarchy.palette) return;

                MHierarchy.palette = ScriptableObject.CreateInstance<MHierarchyPalette>();

                AssetDatabase.CreateAsset(MHierarchy.palette, GetScriptPath("MHierarchy").GetParentPath().CombinePath("mHierarchy Palette.asset"));
            ...
            void getDatas()
            {
                ...
                foreach (var r in gameObjects)
                    goDatas.Add(MHierarchy.GetGameObjectData(r, createDataIfDoesntExist: true));
        ...
        static MHierarchyPalette palette => MHierarchy.palette;
        static MHierarchyData data => MHierarchy.data;
        ...
        {
            instance = ScriptableObject.CreateInstance<MHierarchyPaletteWindow>();
        ...
        public static MHierarchyPaletteWindow instance;
