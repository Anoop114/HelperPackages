...
{
    public class MInspectorComponentHeader
    {
        ...
        void OnGUI()
        {
            void masks()
            {
                ...
                {
                    var defaultButtonCount = MInspectorMenu.componentButtons_defaultButtonsCount;


                    var customButtonCount = 0;

                    if (MInspectorMenu.copyPasteButtonsEnabled)
                        customButtonCount++;

                    if (MInspectorMenu.saveInPlaymodeButtonEnabled && Application.isPlaying)
                        customButtonCount++;


                    if (MInspectorMenu.minimalModeEnabled && !headerRect.IsHovered())
                ...
                void hideArrow()
                {
                    ...
                    if (!MInspectorMenu.minimalModeEnabled) return;
            ...
            {
                if (MInspectorMenu.minimalModeEnabled && !headerRect.IsHovered()) return;
                if (!MInspectorMenu.copyPasteButtonsEnabled) return;


                var copiedData = MInspectorComponentClipboard.instance.copiedComponetDatas.FirstOrDefault(r => r.sourceComponent == component);
                var pastableData = MInspectorComponentClipboard.instance.copiedComponetDatas.FirstOrDefault(r => r.sourceComponent.GetType() == component.GetType());
                ...
                var buttonRect = headerRect.SetWidthFromRight(buttonSize).MoveX(-buttonsOffsetRight - MInspectorMenu.componentButtons_defaultButtonsCount * buttonSize);
                ...
                if (canValuesBePasted)
                    MInspectorComponentClipboard.PasteComponentValues(pastableData, component);
                else
                    MInspectorComponentClipboard.CopyComponent(component);
            ...
            void saveInPlaymodeButton()
            {
                ...
                if (!MInspectorMenu.saveInPlaymodeButtonEnabled) return;


                var savedData = MInspectorComponentClipboard.instance.savedComponentDatas.FirstOrDefault(r => r.sourceComponent == component);

                var isSaved = savedData != null;



                var buttonRect = headerRect.SetWidthFromRight(buttonSize).MoveX(-buttonsOffsetRight - (MInspectorMenu.componentButtons_defaultButtonsCount + (MInspectorMenu.copyPasteButtonsEnabled ? 1 : 0)) * buttonSize);
                ...
                if (editingMultiselection)
                    foreach (var component in multiselectedComponents)
                        MInspectorComponentClipboard.SaveComponent(component);
                else
                    MInspectorComponentClipboard.SaveComponent(component);
            ...
            void expandWithAnimation()
            {
                ...
                if (curEvent.holdingShift)
                    MInspector.CollapseOtherComponents(component, window);

                else if (MInspectorMenu.componentAnimationsEnabled)
                    MInspector.ToggleComponentExpanded(component, window);
            ...
            void createComponentWindow()
            {
                ...
                if (!MInspectorMenu.componentWindowsEnabled) return;

                if (MInspectorComponentWindow.draggedInstance != null) return;
                ...
                MInspectorComponentWindow.CreateDraggedInstance(component, position, headerRect.width);
            ...
            void set_hoveredComponentHeader()
            {
                ...
                if (component is Transform)
                    MInspector.hoveredComponentHeader = null;

                if (headerRect.IsHovered())
                    MInspector.hoveredComponentHeader = this;
        ...
        public void Update()
        {
            ...
            if (imguiContainer != null && imguiContainer.onGUIHandler.Method.DeclaringType == typeof(MInspectorComponentHeader)) return;
        ...
        public MInspectorComponentHeader(Component component, Editor editor) { this.component = component; this.editor = editor; }
