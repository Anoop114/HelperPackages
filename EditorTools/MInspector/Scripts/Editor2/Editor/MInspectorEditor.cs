...
namespace MAttribute
{
    class MInspectorEditor
    {
        ...
        public void OnGUI()
        {
            ...
            void drawMember(MemberInfo memberInfo)
            {
                ...
                void field()
                {
                    ...
                    var isNestedEditor = isSerialized && typesUsingMInspector.Contains((fieldInfo.FieldType));
                    var isResettable = isSerialized && MInspectorResettableVariables.IsResettable(fieldInfo);
                    ...
                    void serialized_resettable()
                    {
                        ...
                        if (!curEvent.isRepaint)
                            if (fieldRects_byLastControlId.ContainsKey(lastControlId))
                                MInspectorResettableVariables.ResetButtonGUI(fieldRects_byLastControlId[lastControlId], serializedProeprty, fieldInfo, targets);
                    ...
                    void serialized_nestedEditor()
                    {
                        ...
                        if (!nestedEditors_byPropertyPath.TryGetValue(serializedProeprty.propertyPath, out var nestedEditor))
                            nestedEditor = nestedEditors_byPropertyPath[serializedProeprty.propertyPath] = new MInspectorEditor(rootPropertyGetter: () => serializedProeprty,
        ...
        public MInspectorEditor(System.Func<SerializedProperty> rootPropertyGetter, System.Func<IEnumerable<object>> targetsGetter)
        {
            ...
            void findTypesUsingMInspector()
            {
                if (typesUsingMInspector != null) return;


                typesUsingMInspector = new();

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<FoldoutAttribute>().Select(r => r.DeclaringType));

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<TabAttribute>().Select(r => r.DeclaringType));

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<ButtonAttribute>().Select(r => r.DeclaringType));
                typesUsingMInspector.UnionWith(TypeCache.GetMethodsWithAttribute<ButtonAttribute>().Select(r => r.DeclaringType));

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<VariantsAttribute>().Select(r => r.DeclaringType));

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<IfAttribute>().Select(r => r.DeclaringType));

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<ReadOnlyAttribute>().Select(r => r.DeclaringType));

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<ShowInInspectorAttribute>().Select(r => r.DeclaringType));

                typesUsingMInspector.UnionWith(TypeCache.GetFieldsWithAttribute<OnValueChangedAttribute>().Select(r => r.DeclaringType));
        ...
        public Dictionary<string, MInspectorEditor> nestedEditors_byPropertyPath = new();
        ...
        static HashSet<Type> typesUsingMInspector;
    ...
    class AbstractEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            rootEditor = new MInspectorEditor(rootPropertyGetter: () => serializedObject.GetIterator(),
                                                   targetsGetter: () => serializedObject.targetObjects);

        }

        MInspectorEditor rootEditor;
...
#if !MINSPECTOR_ATTRIBUTES_DISABLED
