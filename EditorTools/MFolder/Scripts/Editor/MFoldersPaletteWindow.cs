...
using static MFolders.MFolders;
using static MFolders.MFoldersPalette;


namespace MFolders
{
    public class MFoldersPaletteWindow : EditorWindow
    {
        void OnGUI()
        {
        ...
        {
            if (MFoldersData.storeDataInMetaFiles) return; // empties removed from meta files in SaveData()
        ...
        {
            if (!MFoldersData.storeDataInMetaFiles)
                if (data)
                    data.RecordUndo();
        ...
        void MarkDatasDirty()
        {
            ...
            if (MFoldersData.storeDataInMetaFiles)
                MFolders.folderDatasFromMetaFiles_byGuid.Clear();

        }
        void SaveData()
        {
            if (!MFoldersData.storeDataInMetaFiles) { data.Save(); return; }
        ...
        public void Init(List<string> guids)
        {
            void createData()
            {
                if (MFolders.data) return;

                MFolders.data = ScriptableObject.CreateInstance<MFoldersData>();

                AssetDatabase.CreateAsset(MFolders.data, GetScriptPath("mFolders").GetParentPath().CombinePath("mFolders Data.asset"));

            }
            void createPalette()
            {
                if (MFolders.palette) return;

                MFolders.palette = ScriptableObject.CreateInstance<MFoldersPalette>();

                AssetDatabase.CreateAsset(MFolders.palette, GetScriptPath("mFolders").GetParentPath().CombinePath("mFolders Palette.asset"));
            ...
            void getInfos()
            {
                ...
                foreach (var r in guids)
                    folderInfos.Add(MFolders.GetFolderInfo(r, createDataIfDoesntExist: true));
        ...
        static MFoldersPalette palette => MFolders.palette;
        static MFoldersData data => MFolders.data;
        ...
        {
            instance = ScriptableObject.CreateInstance<MFoldersPaletteWindow>();
        ...
        public static MFoldersPaletteWindow instance;
